...
class pyPengIm():
    ...
    def __getattr__(self, name):
        if name in self._dict_paths:
            if name not in self._loaded_dicts:
                self._loaded_dicts[name] = load_dict(self._dict_paths[name])
    ...
    def pinyin(self, text, heteronym=False, accent='tc', auto_split=True):
        if heteronym:
            ...
            if auto_split:
                pinyin_list = self.pinyin_optimize(preprocess_generator(jieba.cut(text)))
            else:
    ...
    def pinyin_optimize(self, word_list):
        ...
        for word in word_list:
            ...
            if not word_translate_flag or not word_found_flag:
...
if __name__ == '__main__':
    with open('output.txt', 'w', encoding='utf-8') as fw:
        pinyin = PyTeochew()
        text = pinyin.to_oral('方便面', auto_split=False)
        print(text, file=fw)
        result = pinyin.pinyin(text, heteronym=True, accent='st')
        print(result)
        print(pinyin.to_phoneme(result['pinyin_seq']))

        print(pinyin.single_query('植'))